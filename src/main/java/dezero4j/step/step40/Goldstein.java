...
public class Goldstein extends LostFunction {
    ...
    public Variable[] forward(Variable... xs) {
        ...
        lost[0] = param(1, xs[0].getShape()).plus(xs[0].plus(xs[1]).plus(param(1, shape)).pow(2).multiply(param(19, xs[0].getShape()).minus(param(14, xs[0].getShape()).multiply(xs[0])).plus(param(3, xs[0].getShape()).multiply(xs[0].pow(2))).minus(param(14, xs[0].getShape()).multiply(xs[1])).plus(param(6, xs[0].getShape()).multiply(xs[0]).multiply(xs[1])).plus(param(3, xs[0].getShape()).multiply(xs[1].pow(2))))).multiply(param(30, xs[0].getShape()).plus(param(2, xs[0].getShape()).multiply(xs[0]).minus(param(3, xs[0].getShape()).multiply(xs[1])).pow(2).multiply(param(18, xs[0].getShape()).minus(param(32, xs[0].getShape()).multiply(xs[0])).plus(param(12, xs[0].getShape()).multiply(xs[0].pow(2))).plus(param(48, xs[0].getShape()).multiply(xs[1])).minus(param(36, xs[0].getShape()).multiply(xs[0]).multiply(xs[1])).plus(param(27, xs[0].getShape()).multiply(xs[1].pow(2))))));
        return lost;
    }
}
