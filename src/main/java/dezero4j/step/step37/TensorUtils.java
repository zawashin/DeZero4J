...
public class TensorUtils {
    ...
    public static int[] indexes(Tensor tensor, int index) {
        // Stub
        int[] indeces = new int[tensor.rank];
        if (tensor.rank == 0) {
            indeces = new int[1];
            indeces[0] = 1;
        } else if (tensor.rank == 1) {
            indeces = new int[tensor.rank];
            indeces[0] = index;
        } else if (tensor.rank == 2) {
            indeces = new int[tensor.rank];
            indeces[0] = tensor.length / tensor.num2x3x4;
            indeces[1] = (tensor.length % tensor.num2x3x4) / tensor.num3x4;
        } else if (tensor.rank == 3) {
            indeces = new int[tensor.rank];
            indeces[0] = tensor.length / tensor.num2x3x4;
            indeces[1] = (tensor.length % tensor.num2x3x4) / tensor.num3x4;
            indeces[2] = ((tensor.length % tensor.num2x3x4) % tensor.num3x4) / tensor.num4;
        } else if (tensor.rank == 4) {
            indeces = new int[tensor.rank];
            indeces[0] = tensor.length / tensor.num2x3x4;
            indeces[1] = (tensor.length % tensor.num2x3x4) / tensor.num3x4;
            indeces[2] = ((tensor.length % tensor.num2x3x4) % tensor.num3x4) / tensor.num4;
            indeces[3] = ((tensor.length % tensor.num2x3x4) % (tensor.num3x4)) % tensor.num4;
        } else {
