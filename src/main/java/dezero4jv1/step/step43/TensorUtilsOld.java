...
public class TensorUtilsOld {
    ...
    public static Tensor reshapeSumBackward(Tensor gy, int[] shape, int axis) {
        if (gy != null) {
            throw new RuntimeException(TensorUtilsOld.ERROR_STUB);
        }
        Tensor x = gy.clone();
        return new Tensor(x);
    }
    ...
    public static Tensor sum(Tensor x) {
        return sum(x, TensorUtilsOld.RANK_MAX);
    }

    public static Tensor sum(Tensor x, int axis) {
        int length;
        int[] sumShape = new int[TensorUtilsOld.RANK_MAX];
        if (axis == TensorUtilsOld.RANK_MAX) {
        ...
        if (axis == TensorUtilsOld.RANK_MAX) {
            for (int i = 0; i < x.shape[0]; i++) {
                for (int j = 0; j < x.shape[1]; j++) {
                    values[0] += TensorUtilsOld.getValue(x, i, j);
                }
            }
        } else if (axis == 0) {
            for (int i = 0; i < x.shape[0]; i++) {
                for (int j = 0; j < x.shape[1]; j++) {
                    values[j] += TensorUtilsOld.getValue(x, i, j);
                }
            }
        } else if (axis == 1) {
            for (int i = 0; i < x.shape[0]; i++) {
                for (int j = 0; j < x.shape[1]; j++) {
                    values[i] += TensorUtilsOld.getValue(x, i, j);
                }
            }
        }
