...
public class LinearLayer extends Layer {
    ...
    public LinearLayer(int numOutputs) {
        super(numOutputs);
        function = new Linear();
    }

    public LinearLayer(int numOutputs, int numInputs) {
        this(numInputs, false, numOutputs);
    }

    public LinearLayer(int numOutputs, boolean noBias, int numInputs) {
        super(numOutputs, numInputs);
        if(!noBias) {
            params.add(new Parameter(TensorUtils.createRandomTensor(1.0/(double)numInputs/(double)numOutputs, numInputs, numOutputs)));
    ...
    public static void main(String[] args) {
        ...
        LinearLayer layer1 = new LinearLayer(numHiddens);
        LinearLayer layer2 = new LinearLayer(numOutputs);
